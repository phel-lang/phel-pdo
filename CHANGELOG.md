...
- `pdo/close`, `pdo/with-connection` and `pdo/closed?` - a connection lifecycle. `close` rolls back an open transaction and marks the handle unusable, so later calls raise instead of reaching PDO; it is idempotent, and it never disturbs a connection wrapped with `pdo/from-connection`, which the host owns. `with-connection` scopes a connection to a body and closes it on a normal return and on a throw alike. PDO has no `close()`, so the socket is released when the last reference to the `\PDO` drops - the docstring says exactly that rather than implying more ([#48]).
...
[Unreleased]: https://github.com/phel-lang/phel-pdo/compare/v0.3.1...HEAD
[0.3.1]: https://github.com/phel-lang/phel-pdo/compare/v0.3.0...v0.3.1
